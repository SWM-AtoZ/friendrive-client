...
import curriculumImg from "./img/curriculum_btnimg.png";
...
function App() {
  ...
  const handleCopyClipBoard = async (uri) => {
    try {
      ...
      if (!isMobile) {
        alert(
          "클립보드에 선생님 페이지가 복사되었습니다. 선생님께 연수를 요청드려보세요!"
        );
      }
    } catch (e) {
  ...
  const ShareTeacher = async () => {
    ...
    if (isMobile) {
      try {
        //플러터 모바일 앱에서 공유하기 실행하는 경우
        if (isTablet && isIOS) {
          //ios tablet 환경에서 사용이 안되는 이슈로 인해 예외처리.
          navigator
            .share({
  ...
  return (
    <section id={style.home_section}>
      ...
      <article className={style.home_article}>
        <button
          ref={Curriculum_btnRef}
          onTouchStart={() => {
            Curriculum_btnRef.current.style.backgroundColor =
              "rgba(0, 0, 0, 0.05)";
          }}
          onTouchMove={() => {
            Curriculum_btnRef.current.style.backgroundColor = "#F8FBFC";
          }}
          onTouchEnd={() => {
            Curriculum_btnRef.current.style.backgroundColor = "#F8FBFC";
          }}
          ...
          className={style.curriculum_btn}
        >
          <div className={style.login_activation}>
            <div className={style.login_textArea}>
              ...
              <div>
                <div>
                  {cookies.token && TotalCheckedNumb
                    ? `${TotalCheckedNumb} / 28 과목 (완주까지 ${progress}%)`
                    : `0 / 28 과목`}
                </div>
                ...
                <div>
                  {cookies.token ? "" : "완주율 확인은 로그인이 필요해요!"}
                </div>
              </div>
            </div>
        ...
        <button
          ref={RequestRef}
          onTouchStart={() => {
            RequestRef.current.style.backgroundColor = "rgba(0, 0, 0, 0.05)";
          }}
          onTouchMove={() => {
            RequestRef.current.style.backgroundColor = "#F8FBFC";
          }}
          onTouchEnd={() => {
            RequestRef.current.style.backgroundColor = "#F8FBFC";
          }}
          ...
          className={style.curriculum_request_btn}
        >
          <div>운전 연수 요청하기</div>
        ...
        <div>
          ...
          <button
            ref={EventRef}
            onTouchStart={() => {
              EventRef.current.style.backgroundColor = "rgba(0, 0, 0, 0.05)";
            }}
            onTouchMove={() => {
              EventRef.current.style.backgroundColor = "#F8FBFC";
            }}
            onTouchEnd={() => {
              EventRef.current.style.backgroundColor = "#F8FBFC";
            }}
          ...
          >
            <div>서비스 피드백</div>
            <div>
              사용중에 좋았던 점 또는 불편했던 점을 남겨주세요<p></p>더 나은
              서비스로 보답 드리겠습니다.
            </div>
          </button>
        </div>
