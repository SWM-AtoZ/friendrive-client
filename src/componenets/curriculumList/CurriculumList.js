...
const CurriculumList = ({ day, title, progress }) => {
  const DaysItemsTotalNumb = [7, 8, 4, 5, 2];
  const TotalNumb = DaysItemsTotalNumb[day - 1];
  const [percentage, setPercentage] = useState("");
  const [cookies, ,] = useCookies(["token"]);
  const navigate = useNavigate();
  const circleRef = useRef();
  const CurriculumListRef = useRef();
  const goTodayList = () => {
  ...
  useEffect(() => {
    ...
    if (cookies.token) {
      ...
      circleRef.current.style.strokeDashoffset =
        outline * (1 - Math.floor((progress / TotalNumb) * 100) / 100);
  ...
  return (
    <button
      ...
      onTouchStart={() => {
        CurriculumListRef.current.style.backgroundColor = "rgba(0, 0, 0, 0.05)";
      }}
      onTouchEnd={() => {
        CurriculumListRef.current.style.backgroundColor = "#F8FBFC";
      }}
      onTouchMove={() => {
        CurriculumListRef.current.style.backgroundColor = "#F8FBFC";
      }}
      onClick={goTodayList}
      className={style.curriculum_list}
    >
      <div className={style.curriculum_progress}>
        {/* dayprocess 의 값에 따라 진행율 적용하여 보여줌 */}
        <div className={style.circle_box}>
          <svg
            className={style.circle_inner_box}
            width="100%"
            height="100%"
            viewBox="0 0 200 200"
            fill="none"
