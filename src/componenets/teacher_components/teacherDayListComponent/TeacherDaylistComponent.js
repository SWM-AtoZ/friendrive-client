...
const TeacherDaylistComponent = ({subject,contents,icon, check}) => {
    ...
    return(
        <button ref={DayListComponentRef} onClick={goToDetail} onTouchStart={()=>{DayListComponentRef.current.style.backgroundColor='rgba(0, 0, 0, 0.05)'}} onTouchMove={()=>{DayListComponentRef.current.style.backgroundColor='#F8FBFC'}} onTouchEnd={()=>{DayListComponentRef.current.style.backgroundColor='#F8FBFC';}}  className={style.daylist}>
